/*
    ...
    ALTER TABLE question_answer_submissions MODIFY score float(5) NOT NULL
    ALTER TABLE question_answer_options MODIFY answer_value float(2) NOT NULL
    ALTER TABLE question_answers MODIFY answer_value float(5) NOT NULL

    mysql> create table ffq_daily_map (
    map_id int not null,
    ...
    mysql> create table daily_thresholds (
 threshold_id int not null PRIMARY KEY AUTO_INCREMENT,
 amount float(2) not null,
 indicator tinyint not null
 );

select q.question_text, qa.answer_score, 
(qa.answer_score * fdm.grams_per_recommended_serving) as units_per_week, 
fdm.daily_question_id, fdm.grams_per_recommended_serving, 
dt.amount as recommended_daily_amount, 
if(
dt.indicator > 0, 
(qa.answer_score * fdm.grams_per_recommended_serving) > dt.amount, 
(qa.answer_score * fdm.grams_per_recommended_serving) < dt.amount
) as success, 
dt.indicator from question_answers qa 
